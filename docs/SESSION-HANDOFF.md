...
1. ⚠️ **Events keyword-monoculture** — the events pipeline only ever queries the generic `"events"` keyword, so stadium mega-events get buried + depth-10-truncated: **World Cup matches one block from Raising Cane's were never fetched.** High business impact. ⛔ **OWNED BY A PARALLEL SESSION** (deep planning, in progress 2026-06-22). Bryan will MERGE that into this handoff before the next session builds. `docs/engine-rewrite/events-keyword-fix-plan.md` here is a STARTING INPUT only — do NOT treat it as final or build from it until the merged plan lands. Decisions (keyword budget / venue source) are the parallel session's to finalize.
