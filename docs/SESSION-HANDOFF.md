...
1. ⚠️ **Events keyword-monoculture** ([[ticket-events-keyword-monoculture]]) — the events pipeline only ever queries the generic `"events"` keyword, so Google Events buries stadium mega-events and depth-10 truncation drops them: **World Cup matches one block from Raising Cane's were never fetched.** Upstream-fetch bug, high business impact (the marquee "smarter than the owner" promise). Fix = marquee-venue radar + keyword expansion + "month" horizon + HIGH_SIGNAL_KEYWORDS. **Strongest next candidate.**
