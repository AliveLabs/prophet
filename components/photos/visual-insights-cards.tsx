...
function SeverityDot({ severity }: { severity: string }) {
  const color =
    ...
    severity === "warning" ? "bg-amber-500" : "bg-sky-500"
      ...
      {hasDistributions && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-slate-900">Visual Content Mix</h3>
          </div>
            ...
            {categoryDistributions.slice(0, 4).map((dist) => {
              ...
              return (
                <div key={dist.competitorName}>
                  <p className="text-[10px] font-semibold text-slate-600">{dist.competitorName}</p>
      ...
      {hasBenchmarks && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-slate-900">Quality Benchmark</h3>
          </div>

          <div className="mt-3 space-y-2">
            {qualityBenchmarks.slice(0, 5).map((bm) => (
              <div key={bm.competitorName} className="flex items-center justify-between">
                <span className="max-w-[60%] truncate text-[11px] text-slate-600">{bm.competitorName}</span>
                <div className="flex items-center gap-2">
          ...
          <p className="mt-2 text-[9px] text-slate-400">% photos with professional lighting</p>
        </div>
      )}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-rose-50">
              <svg className="h-4 w-4 text-rose-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z" />
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 6h.008v.008H6V6z" />
              </svg>
            </div>
            <h3 className="text-xs font-bold text-slate-900">Promotion Activity</h3>
          </div>
            ...
            {promoActivity.filter((p) => p.promoCount > 0).slice(0, 4).map((promo) => (
              <div key={promo.competitorName}>
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-medium text-slate-700">{promo.competitorName}</span>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-sky-50">
              <svg className="h-4 w-4 text-sky-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
              </svg>
            </div>
            <h3 className="text-xs font-bold text-slate-900">Recent Changes</h3>
          </div>
