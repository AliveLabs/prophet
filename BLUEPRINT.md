...
- **Brand resolution:** `brandFromOrigin(origin)` maps the request origin to `Brand = "ticket" | "neat"`. `BRAND_CONFIG[brand]` provides `(source, industry_type, fromAddress, productName, marketingHost)`. Cross-origin from `getticket.ai`/`www.getticket.ai` → `brand="ticket"`, attributed as `source='getticket.ai'`, `industry_type='restaurant'`, sent from `FROM_ADDRESS_TICKET`. Cross-origin from `useneat.ai`/`www.useneat.ai` → `brand="neat"`, attributed as `source='goneat.ai'`, `industry_type='liquor_store'`, sent from `FROM_ADDRESS_NEAT`. Same-origin requests fall back to body-supplied `industry_type` (or default `restaurant`) and `source` (or default `manual`).
- **Logic:** Checks for existing signup -- blocks if `pending`, blocks if `approved` AND auth user still exists (otherwise allows reapply), resets to `pending` if `declined`. Inserts new row with `status: pending`. NO auth user created. UTM fields are stashed as JSON in `waitlist_signups.notes` (no schema migration). Sends brand-aware waitlist confirmation email (awaited; `WaitlistConfirmation` template branches on `brand` prop). Sends brand-aware admin notification to `chris@alivelabs.io` (`WaitlistAdminNotification` template + subject line both include the product name). Fire-and-forget mirror into `marketing.contacts` via `upsertMarketingContact()` so Chris's n8n nurture drip can branch by `industry_type`.
...
*This document was generated from a complete analysis of the Prophet codebase. Last updated May 1, 2026.*
