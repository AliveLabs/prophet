...
>
> **Previously, May 5, 2026:** (Waitlist email bugfix: `app/api/waitlist/route.ts` brand resolver gains a Host-based anti-tamper layer to close a same-origin attribution bug. Resolution order is now `Origin → Host → body`. `brandFromHost()` maps `app.getticket.ai` → `ticket` and `app.useneat.ai` → `neat`; same-origin POSTs (in-app landing page, admin tooling) can no longer mis-attribute a Ticket signup as Neat by spoofing `industry_type='liquor_store'` in the body — host wins. Body still drives resolution on unrecognized hosts (preview deploys, localhost, admin scripts) so dev flexibility is preserved. The user-facing waitlist confirmation send at `app/api/waitlist/route.ts` line 415 now passes `overrideClientEmailPause: true` because a confirmation is a transactional receipt for a user-initiated action, not a marketing drip — it ships even when `CLIENT_EMAILS_ENABLED=false` (PRD May 5 "Option A"). Admin notification (`clientFacing: false`) is unchanged. New structured diagnostic log line `[waitlist] resolved {origin, host, bodyIndustryType, bodySource, originBrand, resolvedBrand, resolvedIndustry, resolvedSource, email}` fires immediately after resolution so any future misfire is visible in Vercel function logs without code changes. New read-only audit script `scripts/audit/check-marketing-contacts.mts` flags rows where `(industry_type, source)` is provably inconsistent with the brand-routing rules (`liquor_store + getticket.ai` or `restaurant + goneat.ai`), cross-references `public.waitlist_signups.business_name` for context, and emits ready-to-paste SQL UPDATE statements for an operator to apply manually. Companion explainer doc at `app/docs/waitlist-email-system.md`.)
>
> **Previously, May 1, 2026:** (Neat lead capture unblocked. `app/api/waitlist/route.ts` is now brand-aware: `useneat.ai` and `www.useneat.ai` added to the CORS allow-list; a unified `brandFromOrigin()` resolver maps the request origin to `Brand = "ticket" | "neat"` and a `BRAND_CONFIG` table threads the brand through `(source, industry_type)` for `marketing.contacts`, the `from` address (`FROM_ADDRESS_TICKET`/`FROM_ADDRESS_NEAT`), confirmation-email subject/template, and admin-notification subject/template. `marketing.contacts.contacts_source_chk` already allowed `'goneat.ai'` so no migration was needed; Neat signups are attributed under `source='goneat.ai', industry_type='liquor_store'` until a future migration adds `'useneat.ai'` and Resend verifies `useneat.ai` as a sending domain. `lib/email/templates/waitlist-confirmation.tsx` and `lib/email/templates/waitlist-admin-notification.tsx` both gain an optional `brand?: 'ticket' | 'neat'` prop with brand-specific copy/signoff (default `'ticket'` for backward compat). The form on `useneat.ai` posts to `https://app.useneat.ai/api/waitlist` (the `app.useneat.ai` alias is already attached to the same Vercel deployment as Ticket — one shared API plane until Neat warrants its own). Same-origin POSTs now derive the brand from a body-supplied `industry_type='liquor_store'` for parity, otherwise default to Ticket.)
>
> **Previously, April 29, 2026:** (Cutover: production app domain moves from `https://www.thevatic.com` to `https://app.getticket.ai`. `getticket.ai` marketing site (Bryan/Chris's `ticket-marketing` Vercel project) is now wired to POST cross-origin into `/api/waitlist`. CORS allow-list + `OPTIONS` handler shipped in `app/api/waitlist/route.ts`; body schema accepts `business_name`, `source`, `industry_type`, and `utm_*` fields. UTM stash persisted as JSON in `waitlist_signups.notes` (no migration). `lib/marketing/contacts.ts` `UpsertMarketingContactInput` gains a `MarketingSource` field forwarded into `marketing.contacts.source`. Domain literals updated across 5 files: `lib/email/templates/layout.tsx` (`https://www.thevatic.com` → `https://app.getticket.ai` fallback), `lib/email/send.ts` (`info@getvatic.com` → `hello@getticket.ai`, with `RESEND_FROM_TICKET` env override for pre-verification), `lib/stripe/client.ts` (`appInfo.url`), `app/api/stripe/webhook/route.ts` (replaced duplicated From literals with `FROM_ADDRESS_TICKET/NEAT` imports), `scripts/stripe/setup.ts` (Customer Portal `business_profile` legal URLs now point at `getticket.ai`/`useneat.ai` marketing sites). Bug fix: `app/api/cron/daily/route.ts:108-110` had an operator-precedence error (`A ?? B ? expr1 : expr2` parses as `(A ?? B) ? expr1 : expr2`) that silently routed the daily refresh job at `https://${VERCEL_URL}` instead of `NEXT_PUBLIC_APP_URL` whenever the latter was set; now uses correctly-grouped fallback chain. Operational runbook for executing the cutover (DNS, Vercel domain alias, Supabase, Stripe, Google OAuth, Resend, PostHog dashboards, smoke tests, Bryan/Chris handoff) is at `app/docs/cutover-app-getticket-ai.md`. Neat is explicitly out of scope; `goneat.ai` From address and `marketing.contacts.contacts_source_chk` allow-list stay until Neat clones from `vatic-core`.)
...
| `NEXT_PUBLIC_APP_URL` | **Yes (prod)** | `app/(auth)/login/actions.ts`, `app/api/stripe/{checkout,portal,webhook}/route.ts`, `app/api/cron/{daily,trial-reminders}/route.ts`, `app/api/waitlist/route.ts`, `app/onboarding/actions.ts`, `app/(dashboard)/competitors/actions.ts`, `app/actions/{user-management,waitlist,admin-management}.ts`, `lib/email/templates/layout.tsx` | Canonical public origin. Production value (Ticket vertical): `https://app.getticket.ai`. Drives every server-built absolute URL: magic-link redirects, Google OAuth callbacks, Stripe success/cancel/return URLs, transactional email links, internal self-fetch URLs in cron jobs, admin notification dashboard URL. Falls back to `http://localhost:3000` in dev. A stale value silently breaks auth and billing — the localhost fallback was historically tolerated only because every callsite that mattered was patched manually. Treat as required in any non-dev environment. |
...
| `MARKETING_CONTACTS_ENABLED` | No | `lib/marketing/contacts.ts`, `app/api/stripe/webhook/route.ts`, `app/api/waitlist/route.ts`, `app/api/marketing/posthog-bridge/route.ts` | Gates the product-side mirror into `marketing.contacts`. When `"true"`, the Stripe webhook upserts `stripe_customer_id` and `status` ('paid' / 'churned') for the org's `billing_email`; `/api/waitlist` upserts new signups with `status='waitlist'`; and `/api/marketing/posthog-bridge` updates `posthog_distinct_id` on the existing row for a signed-in user. Writes are schema-scoped to Chris's `stream1-supabase-schema.sql` v1.3 (`email`, `first_name`, `last_name`, `industry_type`, `status`, `stripe_customer_id`, `posthog_distinct_id`) -- do not add columns here without updating that file. When `"false"` (default), `upsertMarketingContact()` no-ops silently. As of 2026-04-24 the schema is deployed (migrations `20260421225248`, `20260424151835`, `20260424152231`, `20260424152315`) and `marketing` is in the authenticator `pgrst.db_schemas` list, so this flag is safe to flip to `"true"` in Vercel. The doc and the deployed migration sequence are aligned as of v1.3 (2026-04-27): `marketing.contacts.trial_end_date` is trigger-maintained (`marketing.set_trial_end_date()`, BEFORE INSERT OR UPDATE OF `trial_start_date`) and §10 grants the four-table CRUD set + three-view SELECT to `marketing_ops` alongside the existing `service_role` grants. **Phase 3 Day-1 auth posture (2026-04-27 decision):** n8n authenticates to PostgREST with `service_role`. `marketing_ops` (NOINHERIT NOLOGIN, granted to `authenticator`) is provisioned and grant-complete; cutover is purely a credential swap to a JWT with `"role": "marketing_ops"` claim signed by the project's JWT secret -- PostgREST runs `SET LOCAL ROLE` per request, n8n issues no `SET ROLE` itself. Revisit before any credential rotation event or before the n8n operator pool widens past Chris. |
...
| `20260306142531_social_media_bucket_policies.sql` | Storage policies on `storage.objects` for the `social-media` bucket: public SELECT, service-role INSERT/UPDATE |
| `20260307010100_expand_insight_status.sql` | Expands `insights.status` CHECK to: new/read/todo/actioned/snoozed/dismissed |
| `20260331171149_waitlist_admin_setup.sql` | Alter `waitlist_signups` (add `admin_notes`, `reviewed_by`, `reviewed_at`; change status CHECK to pending/approved/declined), create `platform_admins` table with RLS, add `waitlist_signup_id` to `organizations`, seed initial admin |
| `20260331173237_admin_activity_log.sql` | Create `admin_activity_log` table with indexes and RLS `USING (false)` |
...
| `20260429172100_enable_rls_on_stripe_rollout_tables.sql` | Enable RLS on `public.stripe_webhook_events` and `public.trial_reminder_sends` (both shipped without RLS in `20260424182306_stripe_production.sql`); add explicit `*_anon_deny` policies. Closes the two ERROR-level `rls_disabled_in_public` findings from the 2026-04-27 Supabase security advisor. |
| `20260506220255_fix_marketing_contacts_service_role_grants.sql` | Grant `usage` on `marketing` schema + table/sequence privileges on `marketing.contacts` to `service_role` (catch-up for the original Stream 1 schema migration which only granted to `marketing_ops`). |
| `20260520221713_19_mentions_table.sql` | Create `marketing.mentions` (Stream 5 — web mentions captured via Exa, classified by Claude) with brand/sentiment CHECK constraints and partial indexes for `is_prospect`. RLS added later in `20260527220354`. |
| `20260525180019_align_billing_schema.sql` | Add billing columns to `public.organizations` (`trial_started_at`, `trial_ends_at`, `stripe_price_id`, `current_period_end`, `cancel_at_period_end`, `payment_state`); migrate legacy tier names (`starter`→`entry`, `pro`→`mid`, `agency`→`top`); add unique indexes on stripe customer / subscription IDs. |
| `20260525180031_stripe_events_table.sql` | Create `public.stripe_events` append-only audit + idempotency log for Stripe webhook events. RLS enabled with a deny-all policy; only service_role has DML grants. |
| `20260525192924_stream2_outbound_schema.sql` | Stream 2 outbound: create `marketing.outbound_queue`, `marketing.prospects`, `marketing.replies_processed`, `marketing.studio_outbound_pending_approval`, `marketing.shared_domain_daily_counter` + the `v_outbound_queue_depth` view. Grants SELECT to anon/authenticated and full DML to service_role. RLS added later in `20260527220354`. |
| `20260527220354_enable_rls_marketing_stream2.sql` | Enable RLS on the six `marketing` schema tables (`mentions`, `outbound_queue`, `prospects`, `replies_processed`, `studio_outbound_pending_approval`, `shared_domain_daily_counter`) and add `*_anon_deny` policies matching the Stream 1 pattern. Revokes anon/authenticated INSERT/UPDATE on `marketing.mentions`. Closes the six ERROR-level `rls_disabled_in_public` findings from the 2026-05-25 Supabase security advisor. |
...
### `POST /api/waitlist`
- **Auth:** None (public). CORS allow-list (`ALLOWED_ORIGINS`) controls cross-origin POSTs: `https://getticket.ai`, `https://www.getticket.ai`, `https://useneat.ai`, `https://www.useneat.ai`, plus localhost dev ports. `OPTIONS` preflight returns 204 with brand-agnostic `Access-Control-Allow-{Origin,Methods,Headers}` and a 24h `Max-Age`; non-allow-listed origins receive 403 with no CORS headers leaked.
- **Input:** `{ email: string (required), first_name?, last_name?, business_name?, source?, industry_type?, utm_source?, utm_medium?, utm_campaign?, utm_term?, utm_content?, notes? }`. Cross-origin POSTs from a known marketing site have `source` and `industry_type` overridden by the brand resolver (cannot be spoofed across verticals).
- **Brand resolution (3-tier, `Origin → Host → body`):** `BRAND_CONFIG[brand]` provides `(source, industry_type, fromAddress, productName, marketingHost)`. The resolver tries each tier until one matches:
  1. **`brandFromOrigin(origin)`** — cross-origin from `getticket.ai`/`www.getticket.ai` → `brand="ticket"`, attributed as `source='getticket.ai'`, `industry_type='restaurant'`, sent from `FROM_ADDRESS_TICKET`. Cross-origin from `useneat.ai`/`www.useneat.ai` → `brand="neat"`, attributed as `source='goneat.ai'`, `industry_type='liquor_store'`, sent from `FROM_ADDRESS_NEAT`. Body fields are ignored (cannot spoof brand across verticals).
  2. **`brandFromHost(host)` (anti-tamper, added 2026-05-05)** — when Origin doesn't reveal the brand, the request `Host` is the next-best authority. `app.getticket.ai` → `brand="ticket"`, `app.useneat.ai` → `brand="neat"`. Body-supplied `industry_type` is ignored on these hosts. This closes the same-origin attribution bug where the in-app landing page could mis-attribute Ticket signups as Neat by sending `industry_type='liquor_store'` in the body.
  3. **Body fallback** — only when both Origin and Host are unrecognized (preview deploys, localhost, admin tooling), body-supplied `industry_type` (or default `restaurant`) and `source` (or default `manual`) drive resolution. Preserves dev / admin flexibility.
- **Diagnostic log (added 2026-05-05):** A single structured `console.log("[waitlist] resolved", { origin, host, bodyIndustryType, bodySource, originBrand, resolvedBrand, resolvedIndustry, resolvedSource, email })` fires immediately after resolution. Searchable in Vercel function logs as `[waitlist] resolved`; PII is limited to email.
- **Logic:** Checks for existing signup -- blocks if `pending`, blocks if `approved` AND auth user still exists (otherwise allows reapply), resets to `pending` if `declined`. Inserts new row with `status: pending`. NO auth user created. UTM fields are stashed as JSON in `waitlist_signups.notes` (no schema migration). Sends brand-aware waitlist confirmation email (awaited; `WaitlistConfirmation` template branches on `brand` prop). The confirmation send passes `overrideClientEmailPause: true` because it's a transactional receipt for a user-initiated action (PRD May 5 "Option A") — ships even when `CLIENT_EMAILS_ENABLED=false`. Sends brand-aware admin notification to `chris@alivelabs.io` with `clientFacing: false` (`WaitlistAdminNotification` template + subject line both include the product name; the From line is unconditionally `FROM_ADDRESS_TICKET` because the admin send doesn't pass a `from`). Fire-and-forget mirror into `marketing.contacts` via `upsertMarketingContact()` so Chris's n8n nurture drip can branch by `industry_type`.
- **Audit tooling:** `scripts/audit/check-marketing-contacts.mts` flags rows where `(industry_type, source)` is inconsistent with the brand-routing rules (`liquor_store + getticket.ai` or `restaurant + goneat.ai`), cross-references `public.waitlist_signups.business_name`, and emits ready-to-paste SQL UPDATE statements for an operator to apply manually. Read-only; no mutations. Companion doc: `app/docs/waitlist-email-system.md`.
- **Output:** `{ ok: true }` or `{ ok: false, error: string }` (400 missing/invalid email, 409 for duplicates, 500 server error)
