...
describe("ALT-663: tracked-competitor cap is a plan limit, not a constant", () => {
  it("the three paid tiers do NOT share one cap", () => {
    const caps = PAID_TIERS.map((t) => TIER_LIMITS[t].includedCompetitorsPerLocation)
  ...
  it("entry is BELOW the old hardcoded 5, which is how the bug caused silent data loss", () => {
    ...
    expect(TIER_LIMITS.entry.includedCompetitorsPerLocation).toBeLessThan(5)
  })
  ...
  it("the wizard's fresh-signup fallback matches the tier new orgs are created on", () => {
    ...
    // FALLBACK_MAX_TRACKED is TIER_LIMITS.mid.includedCompetitorsPerLocation. If the creation
  ...
  it("an unknown or null tier degrades to entry, so the cap is never permissive by accident", () => {
    ...
    expect(
      TIER_LIMITS[asSubscriptionTier(null)].includedCompetitorsPerLocation
    ).toBeLessThan(TIER_LIMITS.top.includedCompetitorsPerLocation)
  })

  it("suspended tracks nothing", () => {
    expect(TIER_LIMITS.suspended.includedCompetitorsPerLocation).toBe(0)
  })
})
