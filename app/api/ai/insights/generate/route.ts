...
export async function POST(req: Request) {
  try {
    ...
    // route handlers never pass through the (dashboard) layout's deleted_at gate, and this
    // route makes a paid model call). We INSERT via the service-role admin client below
    // (insights INSERT is RLS-gated to owner/admin), so mirror that policy here with
    // isOrgAdmin instead of silently bypassing it — a member-role user can't create insights.
