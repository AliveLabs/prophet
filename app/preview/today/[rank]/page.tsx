// Expanded DETAIL page for one recommendation — the "go deeper, especially on
// evidence" surface (Bryan's call: a true page, not inline). Scaffolded what / how /
// why-we-know: the recommendation up top, the full step-by-step to run it, then the
// real grounded signals behind it. Richer proof (rival posts + numbers + how-to
...
export default async function PlayDetail({ params }: { params: Promise<{ rank: string }> }) {
  ...
  // resolve evidenceRefs -> the real grounded insights behind this play
  ...
  // Refs are TYPE-level (not per-insight), so show the most recent signals of those
  // types — keep a couple per type so the play's actual driver surfaces, not one stale row.
  ...
  return (
    <div className="pv-page pv-detail">
      <Link href="/preview/today" className="pv-back">← Back to your brief</Link>
      ...
      {/* WHY WE KNOW — resolved to the real grounded signals */}
      <div className="pv-section">
        ...
        <p className="pv-soon">In production this goes further: the rival&apos;s actual posts, the engagement numbers, and why each one worked. Not wired into this view yet.</p>
      </div>
    </div>
  )
}
